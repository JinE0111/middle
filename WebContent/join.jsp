	...
	<div class= "container">
		<div class="col-sm-8 join_wrap">
		  <h3><strong>01.로그인정보</strong></h3>
		  	<hr class="blue-hr">
		  <form class="form-horizontal" method="post" onsubmit="valid()"> <!-- 수평으로 정렬된 폼 요소를 나타내며, POST 메서드를 사용하여 데이터를 서버에 전송하고, 제출 시 valid() 함수를 호출합니다. -->
		    <label for="id" class="control-label col-sm-2 blue-background titleset" id=formot> <span class="rq">*</span> 회원아이디 </label>     
		    	
		    	<div class="col-sm-2">	
		        	<input type="text" class="form-control a b" id="id" name="memId" placeholder="아이디 입력"
		        	required pattern="[a-zA-Z0-9]{4,}">  
		      	</div>
		     		<div style="display: flex; justify-content: flex-start; align-items: center;">
	    				<button type="button" id="idChk" class="btn btn-primary">ID 중복확인</button>
	    				<div id="chatlive" style=>
	       			 		<span id="disp"></span>
	    				</div>
					</div>
		      <br>
		    ...
		    <div class="form-group">
		      <label for="password" class="control-label col-sm-2 blue-background poss c titleset"> <span class="rq">*</span> 비밀번호</label>
		      	<div class="col-sm-3">
		        	<input type="password" class="form-control a b" id="password" name="memPw" placeholder="비밀번호 입력"
		     ...
		     <div class="form-group">
		      <label for="name" class="control-label col-sm-2 blue-background c titleset" id="pass2"><span class="rq">*</span>비밀번호 확인</label>
		      <div class="col-sm-3">
		        <input type="password" class="form-control a" id="name5" name="memPw" placeholder="비밀번호 확인"
		        	required pattern="(?=.*\d)(?=.*[a-zA-Z])(?=.*\W).{8,}">
		      </div>
		    </div>
		  <br><br>
		  ...
		  <hr class="blue-hr">
			<div class="form-group">
				<label for="name" class="control-label col-sm-2 blue-background titleset" id="name"><span class="rq">*</span>이름</label>
					<div class="col-sm-2">
				  		<input type="text" class="form-control a" id="name6" name="memName" placeholder="이름입력">
					</div>
					<br>
		<br>
		<div class="form-group">
			<label for="phone1" class="control-label col-sm-2 blue-background top titleset" id="phoenumber"><span class="rq">*</span> 휴대폰번호</label>
...
<div class="form-group y">
    <label for="emailLocalPart" class="control-label col-sm-2 blue-background titleset" id=emaillist><span class="rq">*</span> 이메일</label>
    <div class="col-sm-4">
        <div class="input-group">
    ...
    <div class="col-sm-2">
	  ...
	  <br>
	  <div class="form-group">
		<label for="emailLocalPart" class="control-label col-sm-2 blue-background titleset" id=ad1><span class="rq">*</span>배송주소
		</label>
		<div class="col-sm-3">
	    ...
	    <div class="row justify-content-center">
            <div class="col-sm-3 text-center">
				...
				<a href="Main.do" 	class="btn btn-default btn-wide" id="box2" role="button">취소</a>
            		</div>
				</div>
			</div>
			</form>
...
$(document).ready(function() {
    // 비밀번호 확인 필드에서 키를 누를 때마다 검증
    $('#name5').keyup(function() {
        // 비밀번호와 비밀번호 확인 필드의 값 가져오기
        var password = $('#password').val();
        var confirmPassword = $('#name5').val();

        // 비밀번호와 비밀번호 확인이 일치하는지 검사
        if(password == confirmPassword) {
            $('#passwordWarning').text('비밀번호가 일치합니다.').css('color', 'green');
        } else {
            $('#passwordWarning').text('비밀번호가 일치하지 않습니다.').css('color', 'red');
        }
    });
});
...
function check() {
    ...
    $.ajax({
        type: 'post',
        url: 'membership.jsp', // 서버 측 JSP 파일로 요청을 보냅니다.
...
function validatePassword() {
	  ...
	  if(password.length < 8 || !/[a-zA-Z]/.test(password) || !/[0-9]/.test(password) || !/\W/.test(password)){
	    $('#passwordWarning').text('영문, 숫자, 특수문자 8자리 이상');
	  } else {
